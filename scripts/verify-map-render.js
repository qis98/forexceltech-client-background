...
async function main() {
  ...
  try {
    ...
    const result = await client.call('Runtime.evaluate', {
      returnByValue: true,
      expression: `(() => ({
        active: document.querySelector('#customer-map-view')?.classList.contains('active'),
        hasEcharts: !!window.echarts,
        hasGeojson: !!window.FOREXCELTECH_MAP_GEOJSON?.china,
        chartInstance: !!window.echarts?.getInstanceByDom(document.querySelector('#customer-map-canvas')),
        canvasWidth: document.querySelector('#customer-map-canvas')?.clientWidth,
        canvasHeight: document.querySelector('#customer-map-canvas')?.clientHeight,
        childCount: document.querySelector('#customer-map-canvas')?.children.length,
        ...
        innerStart: document.querySelector('#customer-map-canvas')?.innerHTML.slice(0, 160),
        title: document.querySelector('#map-title')?.textContent,
        summary: document.querySelector('#map-summary')?.textContent.replace(/\\s+/g, ' ').trim()
      }))()`
    });
    console.log(JSON.stringify(result.result.value, null, 2));
    const value = result.result.value;
    if (!value.active || !value.hasEcharts || !value.hasGeojson || (value.canvasCount + value.svgCount) < 1) {
      process.exitCode = 1;
    }
  } finally {
    client?.close();
